<?php
    ...
    if (!empty($_POST)){
        ...
        echo "<script>location.href='pesquisa-alunoAvaliar.php';</script>";
        $mysqli->Close();
        die();        
    }
...
?>
    ...
    <div class="band">
        <div class="container">
            ...
            <?php
                $sqlBusca = "SELECT b.`ban_codigo`, 
                                    ...
                                    WHERE db.`usu_codigo` = ".$Avaliador."
                                        AND db.`band_codigo` NOT IN (SELECT bav.`band_codigo` FROM `banca_detalhe_avaliacao` as bav)";
                ...
                if(mysqli_num_rows($queryBusca) > 0){
                    ...
                    while ($Resultado = $queryBusca->fetch_assoc()) {
                        ...
                        echo '        <td WIDTH="100">'.$Resultado['ban_tipo_nome'].'</td>';
            ...
            ?>
            <br>
            <button class="btn left" id="voltar" name="voltar" type="button" onclick="parent.location='pesquisa-alunoAvaliar.php'"> 
